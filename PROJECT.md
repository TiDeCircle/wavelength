...
**bug ที่เจอตอน play-test บน production** — 3 ตัว แก้แล้วทั้งหมด:

| bug | commit | ทำไมเทสก่อนหน้าไม่เจอ |
|---|---|---|
| co-op คนที่ไม่ใช่คนแรกลากเข็มไม่ได้ (client) | `e98a21b` | role check อ่าน teamId จาก lobby record แทน roster ที่ freeze — โหมด 2 ทีมสองค่าตรงกัน เทส 4 คนเลยผ่าน |
| เหมือนกัน แต่ฝั่ง server | `3cf4a3b` | แก้ client อย่างเดียวไม่พอ server ทิ้ง update เงียบ ๆ เข็มขยับบนจอตัวเองแต่อีกเครื่องไม่เห็น |
| **psychic กดล็อกคำตอบได้** | `a8c4308` | `canLock = isGuessTeam` ไม่ได้ตัด psychic ออก ทั้งที่ psychic รู้คำตอบอยู่แล้ว ตอนเล่นจริงจบรอบตั้งแต่เข็มยังอยู่ที่ 50 ได้ 0 คะแนน |

ตัวที่ 3 เจอได้เพราะเล่นกับคนจริงเท่านั้น — automated test ที่เขียนไว้จะกดตามลำดับที่ถูกอยู่แล้ว
ไม่มีทางกดปุ่มที่ไม่ควรมีสิทธิ์กด
